...
TinyRet tiny_socket_set_nonblock(int fd)
{
    TinyRet ret = TINY_RET_OK;
...
TinyRet tiny_socket_set_block(int fd)
{
    TinyRet ret = TINY_RET_OK;
...
uint32_t tiny_socket_get_ipv4_bytes(int fd)
{
    uint32_t ip = 0;
    ...
    do
    {
        ...
        if (getsockname(fd, (struct sockaddr *)&sin, &len) == 0)
        {
            ...
            LOG_W(TAG, "tiny_socket_get_ipv4_bytes NOT implemented");
            break;
        }
...
TinyRet tiny_socket_get_ip(int fd, char ip[], uint32_t ip_len)
{
    TinyRet ret = TINY_RET_OK;
    ...
    do
    {
        ...
        */
        ...
        if (ret < 0)
        {
            ...
            result = TINY_RET_E_SELECT;
            break;
        }

        if (ret == 0)
        {
            result = TINY_RET_E_TIMEOUT;
            break;
        }

        if (tiny_socket_has_error(fd))
        {
            result = TINY_RET_E_SOCKET_READ;
            break;
        }
...
TinyRet tiny_socket_waiting_for_write(int fd, uint32_t timeout)
{
    ...
    do
    {
        ...
        */
        ...
        if (tiny_socket_has_error(fd))
        {
            result = TINY_RET_E_SOCKET_WRITE;
            break;
        }
...
TinyRet tiny_tcp_open(int *fd, bool block)
{
    *fd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    ...
    return tiny_socket_set_nonblock(*fd);
}
...
TinyRet tiny_tcp_listen(int fd, uint32_t port, uint32_t max_conns)
{
    ...
    if (ret == SOCKET_ERROR)
    {
        return TINY_RET_E_SOCKET_LISTEN;
    }
#else
    if (ret < 0)
    {
    ...
    do
    {
        ...
        else if (ret < 0)
        {
            ...
            if (e == WSAEWOULDBLOCK)
            {
                ...
                result = TINY_RET_PENDING;
                break;
            }
            else if (e == WSAEISCONN)
            {
                LOG_W(TAG, "connection is established!");
                result = TINY_RET_E_SOCKET_CONNECTED;
                break;
            }
            else
            {
                result = TINY_RET_E_SOCKET_DISCONNECTED;
                break;
            }
            break;
#else
...
{
    return tiny_socket_waiting_for_read(fd, timeout);
}

TinyRet tiny_tcp_waiting_for_write(int fd, uint32_t timeout)
{
    return tiny_socket_waiting_for_write(fd, timeout);
}
