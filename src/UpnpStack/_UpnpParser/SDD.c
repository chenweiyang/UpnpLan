...
static CtRet SDD_ParseXml(UpnpService *thiz, TinyXml *xml);
static CtRet SDD_LoadSpecVersion(UpnpService *thiz, TinyXmlNode *root);
static CtRet SDD_LoadActionList(UpnpService *thiz, TinyXmlNode *root);
static CtRet SDD_LoadServiceStateTable(UpnpService *thiz, TinyXmlNode *root);
...
CtRet SDD_Parse(const char *url, UpnpService *service, uint32_t timeout)
{
    ...
    RETURN_VAL_IF_FAIL(service, CT_RET_E_ARG_NULL);
    ...
    do
    {
        ...
        do
        {
            ...
            do
            {
                ...
                if (RET_FAILED(ret))
                {
                    LOG_D(TAG, "SDD_ParseXml failed: %s", ct_ret_to_str(ret));
                    break;
                }
            } while (0);
