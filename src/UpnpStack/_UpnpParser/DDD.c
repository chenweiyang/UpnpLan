...
static CtRet DDD_ParseXml(UpnpDevice *thiz, TinyXml *xml);
static CtRet DDD_LoadSpecVersion(UpnpDevice *thiz, TinyXmlNode *root);
static CtRet DDD_LoadDevice(UpnpDevice *thiz, TinyXmlNode *root);
static CtRet DDD_LoadUrlBase(UpnpDevice *thiz, TinyXmlNode *root);
static CtRet DDD_LoadIconList(UpnpDevice *thiz, TinyXmlNode *iconList);
static CtRet DDD_LoadServiceList(UpnpDevice *thiz, TinyXmlNode *serviceList);
static CtRet UpnpDevice_SetURLBase(UpnpDevice *thiz, const char *deviceUrl);
...
CtRet DDD_Parse(const char *url, UpnpDevice *device, uint32_t timeout)
{
    ...
    RETURN_VAL_IF_FAIL(device, CT_RET_E_ARG_NULL);
    RETURN_VAL_IF_FAIL(url, CT_RET_E_ARG_NULL);
    ...
    do
    {
        ...
        if (RET_FAILED(ret))
        {
            LOG_D(TAG, "UpnpDevice_SetUrlBase failed: %s", ct_ret_to_str(ret));
            break;
        }
        ...
        do
        {
            ...
            if (RET_FAILED(ret))
            {
                LOG_D(TAG, "HttpMessage_Set_GET failed: %s", ct_ret_to_str(ret));
                break;
            }
            ...
            do
            {
                ...
                if (RET_FAILED(ret))
                {
                    LOG_D(TAG, "DDD_ParseXml failed: %s", ct_ret_to_str(ret));
                    break;
                }
            } while (0);
...
static CtRet UpnpDevice_SetURLBase(UpnpDevice *thiz, const char *deviceUrl)
{
    ...
    RETURN_VAL_IF_FAIL(thiz, CT_RET_E_ARG_NULL);
    RETURN_VAL_IF_FAIL(deviceUrl, CT_RET_E_ARG_NULL);

    do
    {
        char url_base[CT_URL_LEN];
        char ip[CT_IP_LEN];
        uint16_t port = 0;
        char uri[512];
        ...
        ret = url_split(deviceUrl, ip, CT_IP_LEN, &port, uri, 512);
        if (RET_FAILED(ret))
        {
            break;
        }

        ct_snprintf(url_base, CT_URL_LEN, "http://%s:%d", ip, port);
