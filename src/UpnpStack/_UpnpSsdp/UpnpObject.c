...
UpnpObject * UpnpObject_New(void)
{
    ...
    do
    {
        ...
        thiz = (UpnpObject *)ct_malloc(sizeof(UpnpObject));
        if (thiz == NULL)
        {
            break;
        }
...
CtRet UpnpObject_Construct(UpnpObject *thiz)
{
    CtRet ret = CT_RET_OK;
...
CtRet UpnpObject_Dispose(UpnpObject *thiz)
{
    RETURN_VAL_IF_FAIL(thiz, CT_RET_E_ARG_NULL);
...
void UpnpObject_Copy(UpnpObject *dst, UpnpObject *src)
{
    ...
    strncpy(dst->location, src->location, CT_URL_LEN);
...
CtRet UpnpObject_SetNt(UpnpObject *thiz, const char *nt, bool strict_uuid)
{
    CtRet ret = CT_RET_OK;

    RETURN_VAL_IF_FAIL(thiz, CT_RET_E_ARG_NULL);
    RETURN_VAL_IF_FAIL(nt, CT_RET_E_ARG_NULL);
...
void UpnpObject_SetLocation(UpnpObject *thiz, const char *location)
{
    ...
    strncpy(thiz->location, location, CT_URL_LEN);
}
...
{
    RETURN_IF_FAIL(thiz);
    thiz->next_notify = (ct_getusec() / 1000 / 1000) + thiz->max_age;
}
